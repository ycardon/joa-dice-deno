...
export abstract class Dice {
  ...
  roll(times = 1): Dice {
    for (let i = 0; i < times; i++) {
      const roll = this.faces[Math.floor(Math.random() * this.faces.length)];
  ...
  add(other: Dice): Dice {
    Faces.map((f) => {
      const sum = (this.result.get(f) || 0) + (other.result.get(f) || 0);
  ...
  toObject() {
    ...
    const dice = { type: this.constructor.name, result: {} as any };
  ...
  applyDefense(defence: Dice): Dice {
    this.lower(
      ...
      this.lower(
        Face.Disrupt,
        this.lower(Face.Kill, defence.result.get(Face.Shield) || 0),
      ),
    );
    return this;
  }
...
export function attack(
  attackDice: Map<Dice, number>,
  defenceDice: Map<Dice, number> = new Map(),
): object {
  const attack = new EmptyDice();
  const defence = new EmptyDice();
